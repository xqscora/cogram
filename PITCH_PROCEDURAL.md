...
| electionguard `words.py` bad input handling | **~206** | `swe_0600…:47–49` — locate + input-handling focus |

**~2.5M-token history → ~200 tokens per recall = 7,000–17,000× smaller than re-reading the trajectories**, and it lands on the exact line, not a paraphrase.
